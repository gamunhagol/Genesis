...
public class EvokerFangsLineSpell extends MagicSpell {
    ...
    protected void onExecute(Level level, LivingEntity caster, DamageSnapshot spellSnapshot) {
        ...
        for (int i = 0; i < 16; ++i) {
            double d0 = 1.25D * (double)(i + 1);
            this.spawnFangs(level, caster, caster.getX() + (double)Mth.cos(f) * d0, caster.getZ() + (double)Mth.sin(f) * d0, caster.getY(), caster.getY() + 1.0D, f, i * 2, spellSnapshot);
        }
    }
